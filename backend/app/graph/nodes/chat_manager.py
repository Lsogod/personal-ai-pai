...
async def _run_tool_agent(
    *,
    ...
    system_prompt = (
        ...
        "只有在需要查询外部状态、读取实时信息、执行外部动作，或你无法根据现有知识可靠回答时，才调用工具。\n"
        "天气查询优先使用 maps_weather；时间查询优先调用 now_time。\n"
        "外部联网查询统一使用 web_search；不要自行编排底层搜索工具。\n"
        "只有在你无法根据现有知识可靠回答，或用户明确要求你查询、搜索、核实时，才调用 web_search。\n"
        "web_search 会返回结构化 JSON，包含 status、answer_ready、summary、sources。\n"
        "如果 web_search 的标题、摘要或正文预览已经提供明确答案，就直接回答，并附上 1 到 3 个实际来源。\n"
        "如果 web_search 返回 no_results、results_low_authority、results_insufficient 或 network_error，就依据该结果如实说明，不要自己编造失败原因。\n"
        "只有 status 明确是 network_error 时，才能说明连接失败、超时或联网异常；其余状态不得误报为网络问题。\n"
        "如果工具报错或结果明显不对题，你只能根据错误信息或上一步结果自我修正一次参数并重试一次；第二次仍不行就停止，并把具体失败原因返回给用户。\n"
