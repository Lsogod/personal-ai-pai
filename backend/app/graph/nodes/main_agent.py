...
def _build_system_prompt(
    *,
    ...
    return (
        ...
        "- 搜索类问题优先使用外部 MCP 搜索工具：先 mcp_list_tools 查看，再 mcp_call_tool 调用。\n"
        "- 新闻/热点类查询：优先抓取新闻频道页或新闻专题页，不要优先抓门户首页。"
        "例如“今日热点新闻”“今天国内新闻”优先抓 https://news.sina.com.cn/ 这类新闻页，而不是 https://www.sina.com.cn/。\n"
        "- 新闻/搜索类问题最多抓取 3 个页面；每抓完一个页面先判断信息是否已经足够，足够就立即停止，不要反复抓取相似页面。\n"
        "- 如果连续尝试后仍未找到可靠来源或明确发布日期，直接说明“未检索到可靠来源”，不要继续循环调用 fetch_url。\n"
