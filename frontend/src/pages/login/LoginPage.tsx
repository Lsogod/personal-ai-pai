...
function validateSubmit(mode: AuthMode, email: string, password: string, code: string, newPassword: string) {
...
export function LoginPage() {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      const validationError = validateSubmit(mode, email, password, code, newPassword);
      if (validationError) {
      ...
      if (mode === "register_code") {
        const data = await apiRequest("/api/auth/register/code", {
          method: "POST",
          body: JSON.stringify({ email: trimmedEmail, password, code: code.trim() }),
        });
        return { kind: "token", token: data.access_token as string };
      }
      const data = await apiRequest("/api/auth/password/reset", {
        method: "POST",
        body: JSON.stringify({ email: trimmedEmail, code: code.trim(), new_password: newPassword }),
      });
