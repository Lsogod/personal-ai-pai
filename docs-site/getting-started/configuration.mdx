...
| `LONG_TERM_MEMORY_TTL` | 记忆有效期（天） | `180`（6 个月） |
| `LONG_TERM_MEMORY_RETRIEVE_LIMIT` | 每次召回的最大记忆条数 | `6` |
| `LONG_TERM_MEMORY_CONTEXT` | 记忆上下文窗口大小（字符数） | `8000` |
| `LONG_TERM_MEMORY_DEBOUNCE` | 记忆提取防抖时间（秒），0 表示不防抖 | `12` |
...
<Tip>
  `MIN_CONFIDENCE` 控制记忆提取的精确度：值越高，提取的记忆越精确但可能遗漏；值越低，召回更全面但可能包含噪声。`feat/single-agent` 分支默认 `0.75`，配合全量注入策略使用较高阈值过滤低质量记忆。
</Tip>
